...
def admin_menu_kb(is_head: bool = False, admins_hidden: bool = False) -> InlineKeyboardMarkup:
    ...
    if is_head:
        ...
        rows.append([InlineKeyboardButton(text="🎁 Подарить Telegram-подарок", callback_data="admin:gift:start",
                                           style=ButtonStyle.SUCCESS)])
        rows.append([InlineKeyboardButton(text="🎁 Подарить подарок Telegram", callback_data="admin:gift:start",
                                           style=ButtonStyle.SUCCESS)])
...
def admin_gift_list_kb(gifts: list, page: int, total_pages: int) -> InlineKeyboardMarkup:
    """gifts: список (gift_id, label, star_count) для текущей страницы."""
    rows = []
    for gift_id, label, star_count in gifts:
        rows.append([InlineKeyboardButton(
            text=f"{label} — {star_count}⭐", callback_data=f"admin:gift:pick:{gift_id}",
            style=ButtonStyle.SUCCESS,
        )])
    nav = []
    if page > 1:
